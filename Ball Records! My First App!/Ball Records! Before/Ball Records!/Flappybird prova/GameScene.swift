...
struct MyVariables {
    ...
    static var yourVariable = Int()
    static var keymore = Bool()
    
    //posso usare anche NSUserDefaults.standardUserDefaults().integerForKey("lastscore") per mettere tutti gli score assieme ma se in futuro ci saranno problemi è meglio avere due cose separate
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func createScene(){
        ...
        difficulty.fontColor = UIColor.greenColor()
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        if gameStarted == false{
            ...
            let spawn5 = SKAction.runBlock({() in self.stellaFunc()})
            
            let spawn6 = SKAction.runBlock({() in self.createBarre5()})
            let spawn7 = SKAction.runBlock({() in self.createBarre6()})
            let spawn8 = SKAction.runBlock({() in self.createBarre7()})
            let spawn9 = SKAction.runBlock({() in self.createBarre8()})
            
            let spawn10 = SKAction.runBlock({() in self.createBarre9()})
            let spawn11 = SKAction.runBlock({() in self.createBarre10()})
            let spawn12 = SKAction.runBlock({() in self.createBarre11()})
            let spawn13 = SKAction.runBlock({() in self.createBarre12()})
            
            let random = SKAction.runBlock({()
                let array = [spawn1,spawn2,spawn3,spawn4]
                let randomIndex = Int(arc4random_uniform(UInt32(array.count)))
                self.runAction(array[randomIndex])
            })
            
            let random2 = SKAction.runBlock({()
                let array = [spawn6,spawn7,spawn8,spawn9]
                let randomIndex = Int(arc4random_uniform(UInt32(array.count)))
                self.runAction(array[randomIndex])
            })
            
            let random3 = SKAction.runBlock({()
                let array = [spawn10,spawn11,spawn12,spawn13]
                let randomIndex = Int(arc4random_uniform(UInt32(array.count)))
                self.runAction(array[randomIndex])
            })
            ...
            let SpawnDelay = SKAction.sequence([
                ...
                random,delay1, random,delay1, random,delay2,
                //4
                spawn,delay,
                ...
                random2,delay1, random2,delay1, random2,delay2,
                //2
                spawn,delay,
                
                spawn5,delay1, random2,delay1, random2,delay2,
                //3
                spawn,delay,
                
                random2,delay1, random2,delay1, random2,delay1, random2,delay2,
                //4
                spawn,delay,
                ...
                random3,delay1, random3,delay1, random3,delay1, random3,delay2,
                //2
                spawn,delay,
            ...
            let distance2 = CGFloat(self.frame.width - 400 + Barre.frame.width) //puoi tolgiere "*distance" ma la velocità cambia
            ...
            if self.nodeAtPoint(location) == self.replay
            {
                ...
                if key1 > 3 {
                    ...
                    if run > 4 {
                        ...
                        let alert = UIAlertController(title: "Ops!", message: "You can't resume the game, you have done too many times! Press Ok for restart the game.", preferredStyle: UIAlertControllerStyle.Alert)
                        alert.addAction(UIAlertAction(title: "Cancel", style: UIAlertActionStyle.Default) { _ in })
